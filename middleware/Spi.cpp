...
void Spi::initialize(SPI_TypeDef *spi, GPIO_TypeDef *gpio, uint16_t gpiopin) {
	...
	// SPIの設定
	{
        ...
        port.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_256;
        /// @todo ボーレートを4に設定
		...
		port.Init.CRCPolynomial = 7;
        /// @fixme CRCPolynomialは7？10？
        port.Init.CRCLength = SPI_CRC_LENGTH_DATASIZE;
        port.Init.NSSPMode = SPI_NSS_PULSE_DISABLE;
		HAL_SPI_Init(&port);
	}

    // HAL_Delay(10);
	// // ピン出力を安定させるために空送信
	// uint8_t bulkdata[2] = {0};
